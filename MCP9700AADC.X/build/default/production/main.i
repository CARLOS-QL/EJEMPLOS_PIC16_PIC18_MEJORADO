...
# 73 "main.c"
    ...
    {
        LCD_cmd(0X02);
        LCD_write_txt("  Temperatura");

        LCD_cmd(0XC0);
        char buffer[10];
        uint16_t val_adc = ADCC_GetSingleConversion(0);
        float voltaje = (5.0/1023)* val_adc;
        float temperatura = (voltaje * 100) - 50;
        sprintf(buffer, "     %3.2f C ", temperatura);
        LCD_write_txt(buffer);
